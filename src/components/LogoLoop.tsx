import { useCallback, useEffect, useMemo, useRef, useState, memo, ReactNode, Key, CSSProperties } from 'react';
import './LogoLoop.css';
...
export const LogoLoop = memo(({
  ...
  const renderLogoItem = useCallback((item: LogoItem, key: Key) => {
    ...
    const content = isNodeItem ? (
      <span className="logoloop__node" aria-hidden={!!(item as LogoNodeItem).href}>{(item as LogoNodeItem).node}</span>
    ) : (
      <img src={(item as LogoImgItem).src} alt={(item as LogoImgItem).alt ?? ''} title={(item as LogoImgItem).title} loading="lazy" decoding="async" draggable={false} />
    );
